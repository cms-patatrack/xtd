...
namespace test::hip {
  ...
  inline void validate_remquo(const Device& device, int ulps = 0) {
    ...
    HIP_CHECK(hipMallocAsync(&result_d, size * sizeof(remquo_t<ResultType>), queue));
    HIP_CHECK(hipMemsetAsync(result_d, 0x00, size * sizeof(remquo_t<ResultType>), queue));
    ...
    HIP_CHECK(
        hipMemcpyAsync(result_h.data(), result_d, size * sizeof(remquo_t<ResultType>), hipMemcpyDeviceToHost, queue));
